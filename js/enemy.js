...
$.Enemy.prototype.render = function (i) {
	if (this.inView) {
		...
		asteroidImage.src = '../asteroid.png'; // Replace with the path to your asteroid image

		$.ctxmg.save();
		$.ctxmg.translate(this.x, this.y);
		$.ctxmg.rotate(mod + Math.PI / 4); // Adjust the rotation angle as needed

		// Draw the asteroid image at the enemy's position
		$.ctxmg.drawImage(
			asteroidImage,
			-this.radius,
			-this.radius,
